...
public class PlayState extends State {
    ...
    private static final int OBSTACLE_SPACING = 30;
    ...
    public PlayState(GameStateManager gsm) {
        ...
        for(int i = 1; i <= OBSTACLE_COUNT; i++) {
            topObstacles.add(new TopObstacle(last_posTop.x + (OBSTACLE_SPACING + TopObstacle.OBSTACLE_WIDTH)));
    ...
    public void update(float dt) {
        ...
        for(BottomObstacle bottomObstacle : bottomObstacles) {
            if(cam.position.x - (cam.viewportWidth / 2) > bottomObstacle.getPosBottom().x + bottomObstacle.getBottomObstacle().getWidth()) {
                bottomObstacle.reposition(bottomObstacle.getPosBottom().x + ((BottomObstacle.OBSTACLE_WIDTH + OBSTACLE_SPACING) * OBSTACLE_COUNT), last_boundsBot);
